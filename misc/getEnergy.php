      ...
      while($row = $result->fetch_assoc()){
       ...
       if (isset($data['error'])) {echo 'Error';}
            ...
            if (isset($data['timestamp'])) {
                $sql = "insert into current_data (userid, name, energy, cooldown_drug, cooldown_booster, refill_energy, refill_nerve) VALUES ('" . $data['player_id'] . "','" . $data['name'] . "','" . $data['energy']['current'] . "/" . $data['energy']['maximum'] . "','" . $data['cooldowns']['drug'] . "','" . $data['cooldowns']['booster'] . "','" . $data['refills']['energy_refill_used'] . "','" . $data['refills']['nerve_refill_used'] . "')";
		echo $sql . "\r\n";
                $conn->query($sql);
            }
       }
